...
@section('content')
    <section>
        <div class="mb-12">
            ...
            @include('partials.breadcrumb', ['breadcrumbs' => ['Dashboard', 'Form Submission', 'Add Field']])
        </div>
        ...
        <div class="space-y-4 md:space-y-6">
            ...
            <form action="{{ route('form-submission.store') }}" method="POST" class="space-y-4 md:space-y-6" >
                @csrf
                <div class="flex flex-wrap gap-8">
                    <div class="flex-1 min-w-64">
                        <label for="fake-name" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Nama Field</label>
                        <input type="text" name="fake-name" id="fake-name" value="{{ old('fake-name') }}" class="bg-gray-50 border border-gray-300 @error('name') border-red-500  @enderror text-gray-900 rounded-lg focus:ring-sky-600 focus:border-sky-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Masukan nama field">
                        <input type="hidden" name="field_name" id="name" value="{{ old('field_name') }}">
                        <div class="show-true-name ms-3 hidden items-center gap-2 mt-1">
                            ...
                            <div class="flex flex-col text-xs">
                                <span class="true-name font-medium text-green-500">Your new field will be created as dimas-wing-bagas</span>
                    ...
                    <div class="flex-1 min-w-64">
                        ...
                        <select id="type" name="type" class="bg-gray-50 border border-gray-300 @error('type') border-red-500  @enderror text-gray-900 rounded-lg focus:ring-sky-600 focus:border-sky-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                            ...
                            @foreach (['text', 'number', 'textarea', 'checkbox', 'select', 'radio'] as $item)
                                <option  value="{{ $item }}" @selected(old('type') == $item) >{{ Str::ucfirst($item) }}</option>
                            @endforeach
                        </select>
                        @error('type')
                            <p class="mt-2 text-sm text-red-600 dark:text-red-500">{{ $message }}</p>
                    ...
                    <div class="flex-1 min-w-64">
                        ...
                        <input type="question" name="question" id="question" value="{{ old('question') }}" class="bg-gray-50 border border-gray-300 @error('name') border-red-500  @enderror text-gray-900 rounded-lg focus:ring-sky-600 focus:border-sky-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Masukan pertanyaan">
                        @error('question')
                            <p class="mt-2 text-sm text-red-600 dark:text-red-500">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div id="min-max-wrapper" class="mt-8 hidden flex-col gap-3">
                    ...
                    <div class="flex items-center flex-nowrap gap-4">
                        <div class="flex-1">
                            ...
                            <input type="number" name="min" id="min" value="{{ old('min') }}" class="key flex-1 bg-gray-50 border border-gray-300 text-gray-900 rounded-lg focus:ring-sky-600 focus:border-sky-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Nilai minimal">
                        </div>
                        <div class="flex-1">
                            <label for="max" class="mb-2 block text-sm font-medium text-gray-900 dark:text-white">Max</label>
                            <input type="number" name="max" id="max" value="{{ old('max') }}" class="key flex-1 bg-gray-50 border border-gray-300 text-gray-900 rounded-lg focus:ring-sky-600 focus:border-sky-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Nilai maksimal">
                        </div>
                    </div>
                </div>
                ...
                <div class="">
                    <label class="inline-flex items-center cursor-pointer">
                        <input name="required" type="checkbox" value="1" class="sr-only peer" {{  old('required') == '1'  ? 'checked' : '' }}>
                ...
                <button type="submit" class=" text-white bg-sky-600 hover:bg-sky-700 focus:ring-4 focus:outline-none focus:ring-sky-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-sky-600 dark:hover:bg-sky-700 dark:focus:ring-sky-800">Add Field</button>
            </form>
        </div>
...
@push('addon-script')
    <script>
        ...
        const oldKey = @json(old('key'));
        const oldValue = @json(old('value'));
